...
post_df %<>%
  ...
  rowwise() %>%
  mutate(text.sg=paste0(segment(paste(title, ptext, sep="。"), mp.seg), collapse=" ")) %>%
  ungroup()
...
bigram <- NGramGenerator(post_df, 2) # 產生文本中所有Bigram及其出現次數

##
bigram <- CandidateSelector(bigram, 2) # 根據出現次數及字串的樣式(pattern)判斷Bigram是否為可能的候選詞語

trigram <- NGramGenerator(post_df, 3) # 產生文本中所有Trigram及其出現次數
...
fourgram <- NGramGenerator(post_df, 4) # 產生文本中所有Fourgram及其出現次數
...
fivegram <- NGramGenerator(post_df, 5) # 產生文本中所有Fivegram及其出現次數
